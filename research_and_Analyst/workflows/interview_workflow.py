...
from research_and_analyst.utils.rate_limiter import invoke_with_retry
...
class InterviewGraphBuilder:
    ...
    def _generate_question(self, state: InterviewState):
        ...
        try:
            ...
            question = invoke_with_retry(self.llm, [SystemMessage(content=system_prompt)] + messages)
    ...
    def _search_web(self, state: InterviewState):
        ...
        try:
            ...
            search_query = invoke_with_retry(structure_llm, [SystemMessage(content=search_prompt)] + state["messages"])

            # Guard: if structured output returned None, retry once
            if search_query is None:
                self.logger.warning("Structured output returned None, retrying...")
                search_query = invoke_with_retry(structure_llm, [SystemMessage(content=search_prompt)] + state["messages"])

            # Fallback: if still None, extract query from the last message
            if search_query is None or not search_query.search_query:
                self.logger.warning("Structured output failed after retry, falling back to last message as query")
                last_message = state["messages"][-1].content if state["messages"] else ""
                fallback_query = last_message[:500] if last_message else state.get("topic", "research topic")
                query_text = fallback_query
            else:
                query_text = search_query.search_query

            self.logger.info("Performing Tavily web search", query=query_text)
            search_docs = self.tavily_search.invoke(query_text)
            ...
            for doc in search_docs:
                if isinstance(doc, dict):
                    ...
                    # Tavily sometimes returns plain strings
                    formatted_docs.append(f'<Document href="#"/>\n{str(doc)}\n</Document>')
            formatted = "\n\n---\n\n".join(formatted_docs)
    ...
    def _generate_answer(self, state: InterviewState):
        ...
        try:
            ...
            answer = invoke_with_retry(self.llm, [SystemMessage(content=system_prompt)] + messages)
            answer.name = "expert"
